...
| D2 | **COCO-pretrained weights: initial 403 was TRANSIENT — now fetched.** A later `curl` of the same `download.pytorch.org` URLs through the same proxy returned HTTP 200 for both `fasterrcnn_mobilenet_v3_large_fpn` and `fasterrcnn_resnet50_fpn_v2` (URLs in `docs/REPRODUCING.md`). | `TorchvisionBallPlayerDetector` now loads genuine COCO weights (`pretrained=True`). **A3 was run** (experiments report): the detector is zero-transfer on the synthetic renders (an appearance domain gap — 0.0 recall/mAP at every resolution) but fires on real frames, giving the first zero-shot **R-regime** data point. The weight-agnostic random-init fallback remains for offline machines. The `.pth` files live in the torch hub cache (~250 MB), **not committed**. |
...
| Hugging Face hub | **Success** — license-gated fetches: `emirsahin/basketball-ball` (MIT, ~215 MB ball-detection images) and a 2-clip sample of `ZhiChengAI/Basketball_V0` (CC BY 4.0, ~17 MB). **Correction (from the A3 zero-shot smoke test):** the Basketball_V0 clips are a *close-up of a tennis ball through a green toy mini-hoop*, not fixed-camera gameplay — usable as a generic "sports ball" negative-ish probe but **not** an R-regime basketball proxy. `emirsahin/basketball-ball` (a person shooting a real ball at a real rim) is the usable real seed. NC/ND/unlicensed candidates inspected and refused. | `scripts/data/hf_download.py` (refuses non-MIT/CC-BY licenses without `--force`) |
